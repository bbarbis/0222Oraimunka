package pkg0222oraimunka;


public class Main {

    
    public static void main(String[] args) {
        Elso10Szamkiirrva();
        KettoSzamKiirva();
        NegyszamOsszegKiirva();
        gyokKivonas();
    }

    private static void gyokKivonas() {
        int a = 3, b=4, c=2;
        osszeg =osszead(osszeg, c);
        s = "%d+%d+%d gyöke:".formatted(a,b,c);
        double gyok = Math.sqrt(osszeg);
        kimenet = String.format("%s %.4f\n", s, gyok);
        kiir(kimenet);
    }

    private static void NegyszamOsszegKiirva() {
        osszeg = osszead(1, 2);
        osszeg = osszead(osszeg, 3);
        osszeg = osszead(osszeg, 4);
        kiir("1+2+3+4="+osszeg+"\n");
    }

    private static void KettoSzamKiirva() {
        int szam1 = 7, szam2 = 4;
        osszeg = osszead(szam1, szam2);
        String s = String.format("%d + %d = %d\n", szam1, szam2, osszeg);
        kiir(s);
    }

    private static void Elso10Szamkiirrva() {
        int osszeg = elso10SzamOsszege();
        String kimenet = "Az első 10 szám összege: " + osszeg + "\n";
        kiir(kimenet);
    }

    private static int elso10SzamOsszege() {
        int osszeg = 0;
        for(int i=0; i<10; i++){
            osszeg += i;
        }
        return osszeg;
    }

    private static int osszead(int a, int b) {
        return a+b;
    }

    private static void kiir(String szoveg) {
        System.out.print(szoveg);
    }
    
}
